...
export function createNoiseGeneratorFromPermutation(permTable) {
    ...
    return function noise(p) {
        const P = new Vector3(Math.floor(p.x), Math.floor(p.y), Math.floor(p.z));
        const p_fract = p.clone().sub(P);
        const f = new Vector3(fade(p_fract.x), fade(p_fract.y), fade(p_fract.z));
...
export class VolumetricMaskController {
    ...
    updateNoiseMaskTexture() {
        ...
        if (!this.noiseTexture) {
            this.noiseTexture = new Data3DTexture(noiseMaskData, noiseMaskSize, noiseMaskSize, noiseMaskSize);
    ...
    updateDetailNoiseMaskTexture() {
        ...
        if (!this.detailNoiseTexture) {
            this.detailNoiseTexture = new Data3DTexture(noiseMaskData, noiseMaskSize, noiseMaskSize, noiseMaskSize);
            this.detailNoiseTexture.format = RedFormat;
            this.detailNoiseTexture.minFilter = LinearFilter;
            this.detailNoiseTexture.magFilter = LinearFilter;
